...
// Strategy: Use target values to learn optimal initial parameters and Dmax strategy
...
// Learn optimal Dmax multiplier from training data
function findOptimalDmaxMultiplier(conc, signal, topStdMean, targetTop, maxIter=6000) {
  ...
  const multipliers = [1.5, 2.0, 3.0, 5.0, 10.0, 20.0, 50.0, 100.0, 200.0, 500.0, 1000.0, 2000.0, 5000.0, Infinity];
  let bestResult = null;
  ...
  return {params: bestResult, multiplier: bestMult, error: bestError};
}

// Train: Find optimal Dmax strategy
console.log('Training: Finding optimal Dmax multipliers to match targets...\n');
...
for (const cytokine of trainingData) {
  ...
  const result = findOptimalDmaxMultiplier(conc, means, topStdMean, target.Top, 6000);
  const params = result.params;
  const midPoint = Math.pow(10, params.Clog);
  
  dmaxMultipliers.push({
    cytokine,
    multiplier: result.multiplier,
    topToMaxSignal: target.Top / Math.max(...means),
    error: result.error
  });
  ...
  console.log(`  Optimal Dmax multiplier: ${result.multiplier}, Error: ${(result.error*100).toFixed(4)}%`);
...
// Analyze learned pattern
const avgMultiplier = dmaxMultipliers.reduce((sum, d) => sum + d.multiplier, 0) / dmaxMultipliers.length;
const medianMultiplier = [...dmaxMultipliers].sort((a,b) => a.multiplier - b.multiplier)[Math.floor(dmaxMultipliers.length/2)].multiplier;
const avgTopToMaxSignal = dmaxMultipliers.reduce((sum, d) => sum + d.topToMaxSignal, 0) / dmaxMultipliers.length;

console.log(`\n📊 Learned Pattern:`);
console.log(`  Average Dmax multiplier: ${avgMultiplier.toFixed(2)}`);
console.log(`  Median Dmax multiplier: ${medianMultiplier.toFixed(2)}`);
...
console.log('\n' + '='.repeat(80));
console.log('\nCalculating for remaining 5 cytokines using learned Dmax strategy...\n');
console.log('='.repeat(80));
...
for (const cytokine of remainingCytokines) {
  ...
  // Estimate target Top based on learned pattern
  const estimatedTop = maxSignal * avgTopToMaxSignal;
  // Use median multiplier for Dmax
  const dmax = estimatedTop * medianMultiplier;
  ...
  const fn = (pArr) => msd_loss4pl({A:pArr[0], D:pArr[1], Clog:pArr[2], B:pArr[3]}, xlog, y, w, hard);
  const res = msd_nelderMead(fn, start, 1.0, 6000, 1e-10);
  const p = {A:res.params[0], D:res.params[1], Clog:res.params[2], B:res.params[3]};
  ...
  console.log(`  HillSlope:  ${p.B.toFixed(4)}`);
  console.log(`  Estimated Top: ${estimatedTop.toFixed(2)}, Dmax: ${dmax.toFixed(2)}`);
}
