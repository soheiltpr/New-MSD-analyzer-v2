// Fit E3P1 4PL parameters COMPLETELY FROM SCRATCH
// Using ONLY E3P1 standard data
...
const msd_nelderMead = (fn, start, step=1.0, maxIter=500, tol=1e-10) => {
  ...
  return {params: simplex[bestIdx], fval: values[bestIdx], iterations: finalIter + 1};
};
...
// E3P1 Standard Curve Data - ONLY from E3P1 plate
...
console.log('                     E3P1 4PL FITTING FROM SCRATCH - USING ONLY E3P1 DATA');
console.log('═'.repeat(120));
console.log('\n✅ Method: 1/y² weighted 4PL with 500+ iterations');
console.log('✅ Data source: E3P1 plate standards ONLY (no reference data)');
console.log('✅ Fitting approach: Nelder-Mead optimization\n');
console.log('─'.repeat(120));
...
for (const [cytokine, data] of Object.entries(E3P1_STANDARDS)) {
  ...
  // Smart initial guesses based on actual data
  const A0 = Math.min(...y);  // Bottom = minimum signal
  const D0 = Math.max(...y);  // Top = maximum signal
  const B0 = 1.0;             // HillSlope typically ~1
  const C0 = Math.sqrt(conc[0] * conc[conc.length-1]);  // Geometric mean of conc range
  ...
  const hard = {
    Amin: 0,
    Dmin: Math.min(...y) * 0.1,  // Allow Bottom to be lower
    Dmax: Math.max(...y) * 10,   // Allow Top to be higher
    Bmin: 0.5,   // MSD typical range
    Bmax: 1.5,   // MSD typical range
    ClogMin, 
    ClogMax
  };
  
  console.log(`   Initial: Bottom=${A0.toFixed(0)}, Top=${D0.toFixed(0)}, MidPoint=${C0.toFixed(2)}, HillSlope=${B0}`);
  ...
  p.D = Math.max(p.D, p.A + 100); // Ensure Top > Bottom by meaningful amount
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    if (calcConc && calcConc > 0 && conc[i] > 0) {
  ...
  console.log(`   📊 Validation: Avg error=${avgErr.toFixed(2)}%, Max error=${maxErr.toFixed(2)}% (${count}/${data.length} points)`);
  ...
  if (avgErr < 5) {
    ...
    console.log(`   ⚠️  POOR FIT (>30% error) - May indicate data quality issues`);
  }
}
...
console.log('\n' + '═'.repeat(120));
console.log('SUMMARY BY CYTOKINE:');
console.log('═'.repeat(120));
console.log('\nCytokine'.padEnd(15) + 'Avg Error (%)'.padEnd(18) + 'Max Error (%)'.padEnd(18) + 'Status');
console.log('─'.repeat(120));
...
for (const [cytokine, data] of Object.entries(E3P1_STANDARDS)) {
  ...
  for (let i = 0; i < data.length; i++) {
    const calcConc = calculate4PLConcentration(signals[i], params);
    if (calcConc && calcConc > 0 && conc[i] > 0) {
      const err = Math.abs((calcConc - conc[i]) / conc[i]) * 100;
      totalErr += err;
      count++;
  ...
  console.log(cytokine.padEnd(15) + avgErr.toFixed(2).padEnd(18) + maxErr.toFixed(2).padEnd(18) + status);
}
