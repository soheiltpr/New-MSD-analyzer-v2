// Final Universal Algorithm for Initial Parameter Estimation
...
// Universal Algorithm
function calculateInitial4PLParams(concentrations, meanSignals, zeroSignals) {
  // 1. Bottom (A0) = mean(zero signals) * 0.9
  ...
  const D0 = maxMeanSignal * 1.01;
  
  // 3. HillSlope (B0) = 1.0 (always)
  const B0 = 1.0;
  
  // 4. MidPoint (C0) = interpolation at (min + max)/2, then multiply by 1.2
  ...
  let C0 = Math.sqrt(concentrations[0] * concentrations[concentrations.length - 1]); // Fallback
  ...
  // Apply correction factor (1.2 works for most, but let's use adaptive)
  // For most cytokines, 1.2 works well. For MCP-1, it's closer to 1.0
  // Let's use 1.2 as default
  C0 = C0 * 1.2;
...
console.log('UNIVERSAL ALGORITHM FOR INITIAL 4PL PARAMETERS');
console.log('='.repeat(80));
console.log('\nFormulas:');
console.log('  Bottom (A0) = mean(zero signals) * 0.9');
console.log('  Top (D0) = max(mean signals) * 1.01');
console.log('  HillSlope (B0) = 1.0');
console.log('  MidPoint (C0) = interpolate at (min+max)/2 signal, then * 1.2');
console.log('\n' + '='.repeat(80));
console.log('VALIDATION AGAINST MSD INITIAL PARAMETERS');
console.log('='.repeat(80));

let totalTopDiff = 0, totalBottomDiff = 0, totalMidPointDiff = 0;
let topMatches = 0, bottomMatches = 0, midPointMatches = 0;
...
for (const [cytokine, data] of Object.entries(allData)) {
  ...
  const calculated = calculateInitial4PLParams(concentrations, meanSignals, data.zero);
  const msd = data.msdInitial;
  
  const topDiff = Math.abs(calculated.D0 - msd.Top) / msd.Top * 100;
  const bottomDiff = Math.abs(calculated.A0 - msd.Bottom) / msd.Bottom * 100;
  const midPointDiff = Math.abs(calculated.C0 - msd.MidPoint) / msd.MidPoint * 100;
  
  totalTopDiff += topDiff;
  totalBottomDiff += bottomDiff;
  totalMidPointDiff += midPointDiff;
  
  if (topDiff < 1) topMatches++;
  if (bottomDiff < 1) bottomMatches++;
  if (midPointDiff < 5) midPointMatches++;
  
  console.log(`\n${cytokine}:`);
  console.log(`  Bottom: Calc=${calculated.A0.toFixed(2)}, MSD=${msd.Bottom.toFixed(2)}, Diff=${bottomDiff.toFixed(2)}% ${bottomDiff < 1 ? '✓' : ''}`);
  console.log(`  Top:    Calc=${calculated.D0.toFixed(2)}, MSD=${msd.Top.toFixed(2)}, Diff=${topDiff.toFixed(2)}% ${topDiff < 1 ? '✓' : ''}`);
  console.log(`  MidPoint: Calc=${calculated.C0.toFixed(4)}, MSD=${msd.MidPoint.toFixed(4)}, Diff=${midPointDiff.toFixed(2)}% ${midPointDiff < 5 ? '✓' : ''}`);
}

console.log('\n' + '='.repeat(80));
console.log('SUMMARY');
console.log('='.repeat(80));
console.log(`Average Top difference: ${(totalTopDiff/10).toFixed(2)}% (${topMatches}/10 within 1%)`);
console.log(`Average Bottom difference: ${(totalBottomDiff/10).toFixed(2)}% (${bottomMatches}/10 within 1%)`);
console.log(`Average MidPoint difference: ${(totalMidPointDiff/10).toFixed(2)}% (${midPointMatches}/10 within 5%)`);
