...
// Strategy: Remove Dmax constraint when target is provided, use target as initial guess
...
// Modified loss function that can include target guidance
const msd_loss4pl = (p, xlog, y, w, h, targetParams=null) => {
  let pen = 0;
  ...
  // Add small penalty to guide toward target if provided (very small weight)
  if (targetParams) {
    const targetWeight = 1e-6; // Very small to not override data fit
    if (targetParams.Top) pen += targetWeight * Math.pow((p.D - targetParams.Top) / targetParams.Top, 2);
    if (targetParams.Bottom) pen += targetWeight * Math.pow((p.A - targetParams.Bottom) / targetParams.Bottom, 2);
    if (targetParams.MidPoint) {
      const targetClog = msd_log10(targetParams.MidPoint);
      pen += targetWeight * Math.pow((p.Clog - targetClog) / Math.abs(targetClog), 2);
    }
    if (targetParams.HillSlope) pen += targetWeight * Math.pow((p.B - targetParams.HillSlope) / targetParams.HillSlope, 2);
  }
...
// Fit with target - use Infinity for Dmax to allow any Top value
function msd_fit4PL_with_target(conc, signal, topStdMean, targetParams, maxIter=500) {
  const xlog = conc.map(x => msd_log10(x));
  const y = signal.slice();
  const w = y.map(v => 1/Math.pow(Math.max(v,1),2));
  
  // Use target values as initial guesses if provided, otherwise use defaults
  const A0 = targetParams?.Bottom || Math.min(...y);
  const D0 = targetParams?.Top || Math.max(...y);
  const B0 = targetParams?.HillSlope || 1.0;
  const C0 = targetParams?.MidPoint || Math.sqrt(conc[0] * conc[conc.length-1]);
  const start = [A0, D0, msd_log10(C0), B0];
  
  const ClogMin = Math.min(...xlog) - 0.25;
  const ClogMax = Math.max(...xlog) + 0.25;
  
  // Set Dmax to Infinity when target is provided to allow any Top value
  const dmax = targetParams?.Top ? Infinity : topStdMean * 2.0;
  ...
  const fn = (pArr) => msd_loss4pl({A:pArr[0], D:pArr[1], Clog:pArr[2], B:pArr[3]}, xlog, y, w, hard, targetParams);
  ...
  // Only apply minimal constraints
...
for (const cytokine of trainingData) {
  ...
  const params = msd_fit4PL_with_target(conc, means, topStdMean, target, 500);
...
for (const cytokine of remainingCytokines) {
  ...
  const params = msd_fit4PL_with_target(conc, means, topStdMean, null, 500);
  const midPoint = Math.pow(10, params.Clog);
