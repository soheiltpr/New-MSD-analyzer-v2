/**
 * Calculate concentrations for IL-5 signals using exact E3P4 parameters from image
 */
...
// Get E3P4 IL-5 parameters
const il5Params = MSD_TRAINING_DATA.E3_P4["IL-5"].params;
const lloq = il5Params["Detection Limits: Calc. Low"];
const uloq = il5Params["Detection Limits: Calc. High"];

console.log('='.repeat(120));
console.log('IL-5 CONCENTRATION CALCULATIONS (E3P4)');
console.log('Using exact parameters from Plate Analysis Properties image');
console.log('='.repeat(120));

console.log('\nParameters:');
console.log(`  Top: ${il5Params["Algorithm Parameter: Calc. Top"]}`);
console.log(`  Bottom: ${il5Params["Algorithm Parameter: Calc. Bottom"]}`);
console.log(`  MidPoint: ${il5Params["Algorithm Parameter: Calc. MidPoint"]}`);
console.log(`  HillSlope: ${il5Params["Algorithm Parameter: Calc. HillSlope"]}`);
...
for (const signal of signals) {
  const concentration = calculateConcentrationFromSignal(signal, il5Params);
...
// Export to CSV
const csvPath = path.join(__dirname, 'il5_calculated_concentrations.csv');
