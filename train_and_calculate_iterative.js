...
// Strategy: Iteratively refine algorithm until training data matches within 0.1%
...
// Try fitting with individual replicates instead of means
function msd_fit4PL_with_replicates(conc, replicates, topStdMean, maxIter=6000) {
  // Flatten replicates: each concentration appears multiple times with its replicate signals
  const xlog = [];
  const y = [];
  const w = [];
  
  for (let i=0; i<conc.length; i++) {
    const xl = msd_log10(conc[i]);
    for (const signal of replicates[i]) {
      xlog.push(xl);
      y.push(signal);
      w.push(1/Math.pow(Math.max(signal,1),2));
    }
  }
  ...
  const dmax = topStdMean * 1000.0; // Very permissive to allow high Top values
...
// Train: Try to match training data
console.log('Training: Attempting to match provided correct answers...\n');
...
for (const cytokine of trainingData) {
  ...
  const replicates = stds.map(s => [s[1], s[2]]); // Use individual replicates
  const means = stds.map(s => msd_mean([s[1], s[2]]));
  const topStdMean = means[0];
  const target = correctAnswers[cytokine];
  
  // Try with replicates
  const params = msd_fit4PL_with_replicates(conc, replicates, topStdMean, 6000);
...
if (!allMatch) {
  console.log(`\n⚠️  Algorithm does not match training data within 0.1%.`);
  console.log(`Using standard MSD algorithm for remaining cytokines.`);
}
...
for (const cytokine of remainingCytokines) {
  ...
  const stds = rows.filter(r => r[0] > 0).sort((a,b) => a[0] - b[0]);
  const conc = stds.map(s => s[0]);
  const replicates = stds.map(s => [s[1], s[2]]);
  const means = stds.map(s => msd_mean([s[1], s[2]]));
  const topStdMean = means[0];
  
  // Use same method as training (with replicates)
  const params = msd_fit4PL_with_replicates(conc, replicates, topStdMean, 6000);
  const midPoint = Math.pow(10, params.Clog);
